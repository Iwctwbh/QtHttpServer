...
void SimpleServers::Run() const
{
	...
	simple_app_crow.catchall_route()([this, &json_object_controllers](const crow::request& request_request) {crow::response response_response{};
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					std::function<void(QJsonObject& arg_json_object_parent, QJsonObject& arg_json_object_child)>function_resolve_data_mapping = [this, &request_request, &json_object_controller,
						&json_object_data, &function_resolve_data_mapping](QJsonObject& arg_json_object_parent, QJsonObject& arg_json_object_child)->void
					{
						std::ranges::for_each(arg_json_object_child.keys(), [this, &request_request, &json_object_controller, &arg_json_object_child, &json_object_data, &function_resolve_data_mapping](const QString& temp_bytearray_child_key) ->void
							{
									...
									regexp.match(temp_bytearray_child_key).hasMatch())
								{
									...
									std::ranges::for_each(temp_json_object_child_child.keys(), [&temp_json_object_child_child, &arg_json_object_child](
										const QString& temp_string_key)->void
										{
								...
								{
									QString temp_string_value{ arg_json_object_child.value(temp_bytearray_child_key).toString() };
									for (const QRegularExpressionMatch& temp_regex_match : regexp.globalMatch(temp_string_value))
									{
					...
					std::ranges::for_each(json_object_response.keys(), [&json_object_response, json_object_data](const QString& temp_string_key)->void
						{
	...
	auto sync_app_crow =
		simple_app_crow
		.bindaddr(json_object_simple_server.value("SimpleServerIpAddress")
			.toString()
			.toLocal8Bit()
			.data())
		.port(json_object_simple_server
			.value("SimpleServerPort")
			.toString()
			.toInt())
		.multithreaded()
		.run_async();
...
void SimpleServers::emit_run() { emit signal_run(); }
