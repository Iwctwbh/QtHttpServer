...
cv::Mat CommonTools::CreateCaptchaImage(std::string *out_string_result)
{
	...
	for (char char_char : string_letter_code)
	{
		...
		double double_font_scale = distribution_int(random_engine_seed) % 2000 / 3000.0 + 1.0;
		uint16_t uint16_thickness = 1;
		int32_t int32_baseline;
		if (cv::Size size_text = cv::getTextSize(string_char, uint16_font_face, double_font_scale, uint16_thickness, &int32_baseline); point_origin.y < size_text.height)
		{
			point_origin.y = uint16_pos_y + distribution_int(random_engine_seed) % 35; //以免超出
		}
		putText(mat_image_copy, string_char, point_origin, uint16_font_face, double_font_scale, cv::Scalar(distribution_int(random_engine_seed) % 255, distribution_int(random_engine_seed) % 255, distribution_int(random_engine_seed) % 255), uint16_thickness, 8, false);
		uint16_pos_x += 45;
