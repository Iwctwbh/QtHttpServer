...
cv::Mat CommonTools::CreateCaptchaImage(std::string *out_string_result) {
  ...
  std::default_random_engine random_engine_seed(
      static_cast<unsigned int>(time(nullptr)));
  std::uniform_int_distribution<> distribution_int(0, 32767);
  cv::Mat mat_image(uint16_image_height, uint16_image_width, CV_8UC3,
  ...
  for (char &i : string_letter_code) {
    i = string_chars[distribution_int(random_engine_seed) %
                     string_chars.length()];
  }
  uint16_t uint16_pos_x{0};
  uint16_t uint16_pos_y{25};
  for (char char_char : string_letter_code) {
    cv::Mat mat_image_copy(uint16_image_height, uint16_image_width, CV_8UC3,
                           cv::Scalar(255, 255, 255));
    ...
    if (cv::Size size_text =
            cv::getTextSize(string_char, uint16_font_face, double_font_scale,
                            uint16_thickness, &int32_baseline);
        point_origin.y < size_text.height) {
      point_origin.y =
          uint16_pos_y + distribution_int(random_engine_seed) % 35;  //以免超出
    }
    putText(mat_image_copy, string_char, point_origin, uint16_font_face,
            double_font_scale,
    ...
    uint16_pos_x += 45;
    cv::Point2f point2f_center(
        mat_image_copy.cols / 2,
        mat_image_copy.rows / 2);  //定义旋转中心为图像的中心
    cv::Mat mat_rot_mat = getRotationMatrix2D(
        point2f_center, distribution_int(random_engine_seed) % 20 - 10, 1);
    warpAffine(mat_image_copy, mat_image_copy, mat_rot_mat,
               mat_image_copy.size());
  ...
  for (int i = 0; i < char_num; i++) {
    ...
    temp_string[0] = string_chars[distribution_int(random_engine_seed) %
                                  string_chars.length()];
    cv::Point origin;
...
QByteArray CommonTools::ConvertMatToBase64(const cv::Mat &arg_mat_image) {
  ...
  cv::imencode(".png", arg_mat_image, buffer);
  const QByteArray bytearray_mat_stream = QByteArray::fromRawData(
